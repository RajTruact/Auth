...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Generate JWT token
        const token = jwt.sign({ email: user[emailCol] }, process.env.JWT_SECRET, { expiresIn: '8h' });
...
function checkUserExists(catalystApp, email) {
    const query = `SELECT * FROM ${tableName} WHERE ${emailCol}='${email}'`;
    return catalystApp.zcql().executeZCQLQuery(query).then(result => result.length > 0);
}
